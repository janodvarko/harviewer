...
DomplateTag.prototype =
{
    ...
    parseAttrs: function(args)
    {
        for (var name in args)
        {
            ...
            {
                var eventName = dojo.isIE ? name : name.substr(2);
                if (!this.listeners)
                    this.listeners = [];
    ...
    compileMarkup: function()
    {
        ...
        var js = dojo.isIE ? 'var f = ' + fnBlock.join("") + ';f' : fnBlock.join("");
...
function defineTags()
{
    for (var i = 0; i < arguments.length; ++i)
    {
        ...
        var fn = new Function("var newTag = new HAR.Domplate.DomplateTag('"+tagName+"'); return newTag.merge(arguments);");

        var fnName = tagName.toUpperCase();
        HAR.Domplate[fnName] = fn;
    }
}
