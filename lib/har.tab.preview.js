...
HAR.Tab.Preview = HAR.extend(
{
    ...
    buildPageContent: function(parentNode, page)
    {
        ...
        EntryList.fileTag.insertRows({files: requests}, lastRow);

        this.updateLayout(this.table, page, requests);
    },

    updateLayout: function(table, page, requests)
    {
        if (!requests)
            requests = HAR.Model.getPageEntries(page);
    ...
    updateTimeline: function(page)
    {
        ...
        for (var row = this.firstRow; row; row = row.nextSibling)
        {
            ...
            var resolvingBar = getElementByClass(row, "netResolvingBar");
    ...
    showInfoTip: function(infoTip, target, x, y)
    {
        ...
        if (row)
        {
            if (getAncestorByClass(target, "netBar"))
            {
