...
HAR.Page.Stats = domplate(
{
    ...
    update: function(page)
    {
        ...
        for (var i=0; i<entries.length; i++)
        {
            ...
            timingPie.data[1].value += entry.timings.connect;
            timingPie.data[2].value += entry.timings.blocked;
            timingPie.data[3].value += entry.timings.send;
            timingPie.data[4].value += entry.timings.wait;
            timingPie.data[5].value += entry.timings.receive;
            timingPie.data[6].value += entry.timings.ssl ? entry.timings.ssl : 0;

            // The ssl time is also included in the connect field
            // (to ensure backward compatibility with HAR 1.1).
            if (entry.timings.ssl > 0)
                timingPie.data[1].value -= entry.timings.ssl;
